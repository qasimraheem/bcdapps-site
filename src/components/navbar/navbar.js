...
},
{
  ...
  <nav className="bcd-nav bcd-nav2">
    <ul>
    {links.map((item) => (<li onClick={() => selectNav(item)} key={item.index}>
        <a className={selectedNav.index === item.index ? 'active-white-item nav-items'  : "nav-items" || item.index === selectedNav.index && item.color === 'black' ? 'makeBlack active-black-item nav-items' : "nav-items"} href={item.link}>
          <div><span className="spans">{item.name}</span></div>
